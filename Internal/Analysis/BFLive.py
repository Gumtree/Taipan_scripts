import sys, os
internal_path = str(os.path.dirname(get_project_path('Internal')))
if not sys.path.__contains__(internal_path) :
...
def batch_export():
    ...
    if not fi.exists():
        if not fi.mkdir():
            logln('Error: failed to make directory: ' + exp_folder)
            return
    HMM_folder = exp_folder + '/HMMfiles'
    fi = File(HMM_folder)
    if not fi.exists():
        if not fi.mkdir():
            logln('Error: failed to make directory: ' + HMM_folder)
            return
